...
- 卡住即时辅导：孩子点“我卡住了”后，Agent 会先给分层提示、引导问题和同类小例子，不直接报答案，并把卡住点纳入后续补漏。
...
- Agent 核心增强：结构化知识点图谱、动态学习策略、标准工具 Schema、Agent Trace、D1/D3/D7/D14 间隔复习。
- Agent 测评体系：`eval_harness` 覆盖学习 Agent 与 Demo Agent，共 89 条 golden case，并输出 JSON/Markdown 报告。
- CI 质量门禁：根目录 `.github/workflows` 已接入 QA Gate 与 Agent Eval。
...
- `POST /api/daily-tasks/{task_id}/event`：孩子端开始、暂停、完成或卡住；卡住时会返回 Agent 分层辅导。
- `GET /api/agent/overview`：查看掌握记录和 Agent 决策日志。
- `GET /api/agent/tools`：查看标准工具 Function Schema。
- `GET /api/agent/strategy`：查看结合薄弱点、卡住、测验结果生成的动态学习策略。
- `GET /api/agent/traces/{trace_id}`：查看某次 Agent 运行的轨迹步骤。
- `POST /api/knowledge/rebuild`：根据 RAG 资料重建知识点图谱。
- `GET /api/knowledge/weak-points`：查看当前薄弱知识点。
- `POST /api/agent/daily-report`：生成带 Agent 结论的日报。

## Agent Eval 与面试材料

- 架构说明：`docs/AGENT_ARCHITECTURE_DIAGRAM.md`
- 测评架构：`docs/AGENT_EVAL_ARCHITECTURE.md`
- 面试总览：`docs/INTERVIEW_README.md`
- 5 分钟演示：`docs/DEMO_SCRIPT_5_MIN.md`
- STAR 案例：`docs/STAR_STORY.md`
- 高频问答：`docs/INTERVIEW_QA.md`
...
python .\eval_harness\runners\eval_runner.py
```

`self_test.py` 覆盖接口、任务流转、出题批改、报告和静态按钮清单；`ui_click_test.py` 会启动临时服务并真实点击管理端、孩子端、家长端的所有按钮；`eval_runner.py` 会生成 `reports/evals/latest_eval_report.json` 与 `reports/evals/latest_eval_report.md`。
